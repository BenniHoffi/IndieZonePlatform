<template>
    <section class="mx-auto w-full max-w-6xl px-4 py-16">
        <div class="text-title-large">Bewertungen</div>
        <div>
            <div class="flex items-center justify-between pt-2 text-secondary">
                <div class="flex items-center">
                    <div v-for="index in 5">
                        <Icon v-if="product.rating >= index" name="material-symbols:star" />
                        <Icon v-else-if="product.rating == index - 0.5" name="material-symbols:star-half" />
                        <Icon v-else name="material-symbols:star-outline" />
                    </div>
                    <!-- TODO: das shop.rating muss noch fett-gedruckt werden -->
                    <div class="pl-2 text-label-medium text-primary">{{ product.rating }}</div>
                </div>
                <Button text @click="">
                    <template #icon>
